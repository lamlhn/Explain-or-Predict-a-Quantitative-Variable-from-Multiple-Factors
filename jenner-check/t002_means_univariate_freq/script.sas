/* Adapted from SAE.sas: this is the author's descriptive-statistics block
   (PROC MEANS / PROC UNIVARIATE with HISTOGRAM+NORMAL / PROC FREQ on all
   character variables), run unmodified against a small inline sample built
   in the same shape as the author's merged TAB_NBA_Final (Salaire cleaned
   from a comma-formatted string, one row per player). */
...
/* Analyse de la distribution des variables numeriques */
...
/* Analyse des variables qualitatives (caracteres) */
