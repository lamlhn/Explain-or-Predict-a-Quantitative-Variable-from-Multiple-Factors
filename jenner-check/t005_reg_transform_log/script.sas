/* Adapted from SAE.sas: the author's dependent-variable transformation
   block (sqrt, cube root, log of Salaire) plus the log-salary regression
   with VIF/CLB/R/INFLUENCE diagnostics, run unmodified against a small
   inline sample shaped like the author's merged TAB_NBA_Final. */
...
PROC UNIVARIATE DATA=residus_racine_salaire NORMAL;
    VAR res; /* Analyse des residus pour tester la normalite */
    HISTOGRAM / NORMAL; /* Histogramme des residus avec une courbe normale superposee */
RUN;

/* Modele log avec 2 variables : Age, W */
