/* Adapted from SAE.sas: the author's PROC REG with selection=rsquare, run
   unmodified against a small inline sample shaped like the author's merged
   TAB_NBA_Final. The predictor list is a subset of the author's full 29-var
   MODEL statement (Classement..FTA) so the best-subsets search completes
   quickly on this sample; the technique — R2-based subset selection with
   OUTPUT OUT=...R=res — is exactly the author's. */
...
/* Regression lineaire multiple avec selection par R2 (sous-ensemble des variables explicatives) */
PROC REG DATA=work.TAB_NBA_Final;
    MODEL Salaire = Classement Age GP W L Min PTS FGM FGA FG_ _3PM _3PA _3P_ FTM FTA
                    / selection = rsquare;
