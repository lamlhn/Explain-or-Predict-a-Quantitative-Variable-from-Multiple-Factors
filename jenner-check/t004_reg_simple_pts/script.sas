/* Adapted from SAE.sas: the author's simple-regression diagnostics block
   (Salaire on PTS, with CLB/R/INFLUENCE/DWPROB diagnostics and a residual
   PROC UNIVARIATE), run unmodified against a small inline sample shaped
   like the author's merged TAB_NBA_Final. */
...
/* Regression simple : Salaire en fonction de PTS */
